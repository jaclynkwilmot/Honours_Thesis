...
summary_data <- data %>%
  group_by(Plot, Month) %>%
  summarise(FloweringCount = sum(Flowering, na.rm = TRUE),
            .groups = "drop")

# Plot flowering rates
ggplot(summary_data, aes(x = Month, y = FloweringCount, 
                         group = Plot, color = Plot)) +
  geom_line(size = 1) +
  geom_point(size = 3) +
  labs(title = "",
       x = "Month of Observation",
       y = "Number of Flowering Plants (out of 40)",
  color = "Treatment") +  
  ...
  theme(text = element_text(family = "Times New Roman", size = 14),
        ...
        panel.grid.major.y = element_line(),   # optional: remove vertical lines
